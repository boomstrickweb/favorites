...
export default function FollowersScreen() {
  ...
  const fetchFollowers = useCallback(async () => {
    ...
    try {
      ...
      // Check privacy of the user whose followers we are trying to see
      ...
      let followers = data.map((item: any) => item.follower).filter(Boolean);

      if (authUser) {
        // Fetch which of these followers the current user is following
        const { data: followingData } = await supabase
          .from('follows')
          .select('following_id')
          .eq('follower_id', authUser.id)
        ...
        // Fetch which of these followers are following the current user
