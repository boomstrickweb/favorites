...
async function fetchProxied(url: string, headers: Record<string, string> = {}) {
  try {
    ...
    if (error) {
      ...
      // Ensure we have a User-Agent when falling back on mobile/web
      const finalHeaders = {
...
export async function searchDishes(query: string): Promise<SearchResult[]> {
  if (!query) return [];
  try {
    const data = await fetchProxied(`https://www.themealdb.com/api/json/v1/1/search.php?s=${encodeURIComponent(query)}`);
    if (!data) return [];
    return (data.meals || [])
...
export async function searchDrinks(query: string): Promise<SearchResult[]> {
  if (!query) return [];
  try {
    const data = await fetchProxied(`https://www.thecocktaildb.com/api/json/v1/1/search.php?s=${encodeURIComponent(query)}`);
    if (!data) return [];
  ...
  try {
    ...
    if (data && data.Results && data.Results.length > 0) {
      ...
      await Promise.all(makeNames.map(async (makeName: any) => {
        try {
          const pexelsData = await fetchProxied(`https://api.pexels.com/v1/search?query=${encodeURIComponent(makeName + " logo")}&per_page=1`, {
